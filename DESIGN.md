...
The page is a single centered column — `min-height: 100dvh`, flex column, natural document scroll — with a top strip, a centered main block, and a footer. The top strip is a baseline-aligned row (board mark left, language switch right) padded 1rem vertically and clamp(1.2rem, 4vw, 3rem) horizontally — no enclosure, just baselines. The main block centers on both axes via `margin-block: auto`, with a 1.1rem gap between its elements and 2rem / 1.2rem padding; the queue inside it stacks with a .8rem gap (1rem above the queue), and the contact row wraps with a 1.2rem × 2.6rem gap, 1.2rem below the asides. The probe form (name, email, message, TRANSMITIR) follows the contact row at the same text size, its fields riding baselines under 1px dust-3 underlines; focus is the amber signal. The footer is a single centered line padded 1.2rem. Mobile-first: the base styles carry the narrow-shed tracking and the page scrolls whenever the content exceeds the viewport; desktop (≥641px) recovers the full letterspacing depth ladder, and at 1440×900 the composition still fits one viewport with no scroll.
